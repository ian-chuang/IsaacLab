...
def main():
    ...
    env_cfg: StackEnvCfg = parse_env_cfg(
        "Isaac-Stack-UR5e-IK-Abs-v0",
        use_gpu=not args_cli.cpu,
        num_envs=args_cli.num_envs,
        use_fabric=not args_cli.disable_fabric,
    )
    # create environment
    env = gym.make("Isaac-Stack-UR5e-IK-Abs-v0", cfg=env_cfg)
    ...
    gripper = SurfaceGripper(
        end_effector_prim_path=f"/World/envs/env_0/Robot/tcp", translate=0.01, direction="z"
    )
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            if count < 100:
                ...
                actions[:, :3] = torch.tensor([[0.3, 0.0, 0.2]], device=env.unwrapped.device)
            elif count == 200:
                gripper.open()
                env.reset()
                count = 0
